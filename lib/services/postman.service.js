...
service.generate = (routes, modelDefinitions, options) => {
    ...
    Object.keys(routes).sort().map(prop => {
...
const generateFolderForModel = (modelName, modelRoutes, modelDefinition, options) => {
    ...
    if (modelName === 'User') {
        const loginRoutes = generateLoginRouteItems(modelRoutes.find(mr => mr.name === 'Login'), modelDefinition, options);
        if (Array.isArray(loginRoutes)) {
            result.item = loginRoutes.concat(result.item);
        }
    }
    return result;
}
...
const generateItemForRoute = (modelRoute, modelDefinition, options) => {
    return {
        name: modelRoute.model
            + ' - ' + modelRoute.name + getPassportStrategy(util.firstLower(modelRoute.name), modelDefinition)
            + (modelRoute.auth ? ` ${modelRoute.auth.map(authLevel => `(${authLevel})`).join(' ')}` : ''),
...
const generateHeader = (modelroute) => {
    ...
    if (modelroute.defineToken || modelroute.auth) {
        ...
        if (modelroute.method === 'POST' || modelroute.method === 'PUT') {
            ...
            for (const prop in model.schema.obj) {
                if (modelDefinition.properties[prop]
                    ...
                    addPropertyToBody(propertiesParent, prop, modelDefinition.properties[prop], model.schema.obj[prop], modelroute.name === 'Search');
                }
            }
        }
    }
...
const addPropertyToBody = (rawObj, propertyName, modelDefinition, type, isSearch) => {
    if (isSearch && typeof type === 'string') {
        ...
        if (type.toLowerCase() === 'number' || type.toLowerCase() === 'date') {
            if (!modelDefinition.disableSearchMinMax) {
                rawObj[propertyName + 'Min'] = null;
                rawObj[propertyName + 'Max'] = null;
            } else {
                rawObj[propertyName] = null;
            }
        }
        return;
    }
...
const getPassportStrategy = (routeName, modelDefinition) => {
    if (!modelDefinition.authLevel || !modelDefinition.authLevel.methods[routeName] || modelDefinition.authLevel.methods[routeName].passportStrategy === 'public') {
        return ''
    }
