...
service.generate = (routes, modelDefinitions, options) => {
    ...
    for (const prop in routes) {
        config.item.push(generateFolderForModel(prop, routes[prop], modelDefinitions, options));
    }
    return config;
}

const generateFolderForModel = (modelName, modelRoutes, modelDefinitions, options) => {
    return {
        name: modelName,
        description: '',
        item: modelRoutes.map(mr => { return generateItemForRoute(mr, modelDefinitions, options) })
    }
}

const generateItemForRoute = (modelRoute, modelDefinitions, options) => {
    return {
