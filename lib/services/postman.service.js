...
const generateLoginRouteItems = (modelRoute, modelDefinition, options) => {
    ...
    if (!options.example_users || !options.example_users === {}) {
        loginItems.push(generateItemForRoute(modelRoute, modelDefinition, options));
    }

    for (const prop in options.example_users) {
        const customModelRoute = {
            name: 'Login as ' + prop,
            custom_postman_request: {
                url: {
                    query: [
                        { key: 'email', value: options.example_users[prop].email },
                        { key: 'password', value: options.example_users[prop].password }
                    ]
                }
            }
        }
