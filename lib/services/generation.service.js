...
const populate = (model, modelDefinition, options) => {
    return new Promise((resolve, reject) => {
        const dataPath = utilService.checkFile(options.data_path, '/' + modelDefinition.route + '.json', null);
        if (!dataPath) { resolve(); }
        const data = require('../../' + options.data_path + '/' + modelDefinition.route);
        const createFunction = require(utilService.checkFile('../../' + options.data_path, './creation/' + modelDefinition.route + '.js', '../generation/creation/default'));
        model.remove({})
            .then(res => {
                let counter = 0;
