...
service.createRoutes = (app, modelDefinitions, options) => {
    util.modelDefinitionsSingleLevel(modelDefinitions).forEach(md => {
        createRoutes(app, md);
    });
    service.printRoutes(app);
    require('../routes/postman')(app, postmanService.generate(app.routesInfo, util.modelDefinitionsSingleLevel(modelDefinitions), options));
}
