...
service.generateModels = (modelDefinitions, options, customValidations) => {
    modelDefinitions.filter(md => md.name !== 'User').forEach(md => {
        storeModel(md, options, customValidations);
    });
    return models;
}
...
const storeModel = (modelDefinition, options, customValidations) => {
    ...
    Object.entries(modelDefinition.properties).forEach(([key, obj]) => {
        objSchema[key] = transformToSchemaProp(obj, customValidations);
    });
...
const transformToSchemaProp = (prop, customValidations) => {
    ...
    // Check validations
    if (prop.validate) {
        prop.validate = customValidations[prop.validate];
    }
