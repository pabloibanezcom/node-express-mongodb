...
const getPropertiesFromObject = (propertiesObj, objSchema) => {
    for (const prop in propertiesObj) {
        if (Array.isArray(propertiesObj[prop])) {
            objSchema[prop] = [];
            objSchema[prop].push(getMongooseSchemaType(propertiesObj[prop][0]));
        } else {
...
const getMongooseSchemaType = (prop) => {
    if (prop && prop.type) {
        if (util.checkIfType(prop.type)) {
            return prop.type;
        } else {
            ...
            return { type: mongoose.Schema.Types.ObjectId, ref: prop.type };
        }
    }
    if (prop && typeof prop === 'object' && !prop.type) {
        ...
        for (const subprop in prop) {
            complexObj[subprop] = getMongooseSchemaType(prop[subprop]);
        }
        return complexObj;
    }
    return {};
...
service.updateRelationships = async (newDoc, doc, relatinoships) => {
    if (!relatinoships) { return; }
    for (let rel of relatinoships) {
