...
module.exports = (app, modelDefinition, model, passport) => {
    ...
    const createRouteForOperatorAndModel = (operator, otherModel, modelDefinition) => {
        if (otherModel.name === 'Custom') {
            otherModel.properties.forEach(p => createRouteForOperatorAndGeoType(operator, p, modelDefinition));
        } else {
            otherModel.properties.forEach(p => createRouteForOperatorAndModelByProperty(operator, otherModel.name, p, modelDefinition));
        }
    }
    ...
    const createRouteForOperatorAndModelByProperty = (operator, otherModelName, property, modelDefinition) => {
        ...
        app.get(url, (req, res) => {
            ...
            modelsService.getModel(otherModelName).find(searchObj)
                .then(result => {
                    ...
                    geoService.runOperator(operator, model, result[0].geometry, toExclude)
        ...
        routesService.storeRoute(app, {
            ...
            name: util.firstUpper(operator) + 'With' + otherModelName + 'By' + util.firstUpper(plainProperty),
            method: 'GET', url: url
        });
    }
    ...
    const createRouteForOperatorAndGeoType = (operator, geoType, modelDefinition) => {
        ...
        app[urlInfo.method.toLowerCase()](url, (req, res) => {
            ...
            geoService.runOperator(operator, model, geometry)
                .then(obj => res.send(obj))
                .catch(err => res.status(500).send(err));
        });
